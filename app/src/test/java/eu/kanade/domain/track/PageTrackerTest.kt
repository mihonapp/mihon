...
class PageTrackerTest {
    companion object {
        ...
        private fun createTestChapterEntry(
            localId: Int,
            localRead: Boolean,
            localPage: Int,
            remoteRead: Boolean,
    ...
    private fun testSampleWithStrategy(strategy: Int) {
        ...
        println(
            "Update to local : ${result[SyncChapterProgressWithTrack.RemoteProgressResolution.ACCEPT]?.map {
                "${it.key.name}  ${it.value.compareWith(it.key.progress)}"
            }}",
        )
        println(
            "Update to remote : ${result[SyncChapterProgressWithTrack.RemoteProgressResolution.REJECT]?.map {
                "${it.key.name}  ${it.key.progress.compareWith(it.value)}"
            }}",
        )
        println(
            "No change : ${result[SyncChapterProgressWithTrack.RemoteProgressResolution.SAME]?.map {
                it.key.name
            }}",
        )
    }
