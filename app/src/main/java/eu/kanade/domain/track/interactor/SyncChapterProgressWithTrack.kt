...
class SyncChapterProgressWithTrack(
    ...
    companion object {
        ...
        private fun syncStrategyAcceptAll(
            ...
            return if ((local.completed && remote.completed) || local.page == remote.page) {
                RemoteProgressResolution.SAME
            } else {
        ...
        private fun syncStrategyAllowReread(
            ...
            return if (local.completed && !remote.completed && remote.page > 1) {
                RemoteProgressResolution.ACCEPT
            } else {
                syncStrategyDefault(local, remote)
            }
        }
    ...
    suspend fun await(
        ...
        try {
            if (tracker is PageTracker) {
                ...
                val updatesToLocal =
                    remoteUpdatesMapping[RemoteProgressResolution.ACCEPT]?.mapNotNull { (chapter, remote) ->
                        if (remote.page > 1 &&
                            (chapter.last_page_read != remote.page - 1 || chapter.read != remote.completed)
                        ) {
                            // In komga page starts from 1
                            chapter.toDomainChapter()?.copy(
                                lastPageRead = remote.page.toLong() - 1,
