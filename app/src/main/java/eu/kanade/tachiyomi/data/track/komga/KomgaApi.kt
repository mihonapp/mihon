    ...
    internal suspend fun getAllBooksOfSeries(v1UrlBase: String, seriesId: String): List<BookDtoPartial> {
        with(json) {
            return client.newCall(
                GET("$v1UrlBase/series/$seriesId/books?unpaged=true", headers),
            ).awaitSuccess().parseAs<SeriesBookListDtoPartial>().content
                ?: listOf()
        }
    }
    ...
    internal suspend fun updateBookProgress(bookUrl: String, pageIndex: Int = 0, complete: Boolean = false) {
        ...
        val resp = client.newCall(
            Request.Builder()
                ...
                .patch(
                    "{\"page\": ${pageIndex + 1}, \"completed\": $complete }".toRequestBody(
