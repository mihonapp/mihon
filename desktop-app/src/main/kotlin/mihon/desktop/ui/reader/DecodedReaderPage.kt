...
fun DecodedReaderPage(
    ...
    var frame by remember(page.id) { mutableStateOf<DesktopReaderFactory.PageFrame?>(null) }
    var failure by remember(page.id) { mutableStateOf<String?>(null) }
    LaunchedEffect(factory, page.id, foreground) {
        var owned: DesktopReaderFactory.PageFrame? = null
        try {
            var index = 0
            while (true) {
                val replacement = factory.loadFrame(page.id, index)
                val previous = owned
                owned = replacement
                frame = replacement
                previous?.tile?.close()
