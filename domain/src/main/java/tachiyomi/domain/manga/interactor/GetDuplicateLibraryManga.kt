...
class GetDuplicateLibraryManga(
    ...
    suspend operator fun invoke(manga: Manga): List<MangaWithChapterCount> {
        return when (libraryPreferences.duplicateMatchLevel().get()) {
    ...
    suspend fun exactTitleMatch(manga: Manga) : List<MangaWithChapterCount> {
        return mangaRepository.getDuplicateLibraryManga(manga.id, manga.title)
    }

    suspend fun fuzzyTitleSearch(manga: Manga) : List<MangaWithChapterCount> {
        val regex = Regex("\\b"+manga.title+"\\b", option = RegexOption.IGNORE_CASE)
        return mangaRepository.getDuplicateLibraryManga(manga.id, '%' + manga.title + '%')
            .filter { regex.containsMatchIn(it.manga.title) }
    }

    suspend fun fuzzySubstringSearch(manga: Manga) : List<MangaWithChapterCount> {
        return mangaRepository.getDuplicateLibraryManga(manga.id, '%' + manga.title + '%')
    }
}
