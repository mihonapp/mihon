...
abstract class HttpSource : CatalogueSource {
    ...
    /**
     * ID of the source. By default, it uses the following code snippet to generate the ID.
     * ```
     * val key = buildString {
     *     append(name.lowercase())
     *     append("/")
     *     // To keep id consistent with the migration from `lang` to `language`
     *     when (language) {
     *         "mul" -> append("all")
     *         "und" -> append("other")
     *         else -> append(lang)
     ...
     * ```
     *
     * `$lang` is a legacy value deprecated in favor of [language]. To get lang replace mul and und with all and other respectively
     *
    ...
    override val id: Long by lazy {
        val key = buildString {
            append(name.lowercase())
            append("/")
            // TO keep id the same after the migration from `lang` to `language`
            when (language) {
                "mul" -> append("all")
                "und" -> append("other")
                else -> append(language)
            }
            append("/")
            append(versionId)
        }
