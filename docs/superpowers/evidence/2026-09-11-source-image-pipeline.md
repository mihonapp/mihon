...
The same-version MSI repair returned 1603 during SecureRepair/CreateShortcuts (elevation unavailable in silent mode). This was not reported as an installer success. With Mihon closed, changed application files were backed up and copied into `%LOCALAPPDATA%/MihonW`; every file from the new distribution was verified against the installed counterpart by SHA-256. The final update completed at 22:48 on September 11. User database, extensions and preferences were not replaced. Backups: `%LOCALAPPDATA%/MihonW-repair-backup-20260911-160855` and `%LOCALAPPDATA%/MihonW-repair-backup-20260911-224845`.
